        ...
        with st.popover("➕", use_container_width=True):
            ...
            if uploaded_file is not None:
                if st.button("Index File", use_container_width=True):
                    ...
                    status = st.status("Indexing document...", expanded=True)
                    tmp_path = None
                    try:
                        ...
                        status.write("✂️ Splitting into overlapping chunks...")
                        status.write("🧠 Generating embeddings & building BM25 index...")
                        pipeline.process_uploaded_file(tmp_path)
                        status.update(label="Document indexed successfully!", state="complete", expanded=False)

                        # Update state and increment counter on success
                        ...
                        # Clean UI alert for image-only or unreadable PDFs
